...
public class UnitFactory {
    ...
    public int getUnitCost(String unitType) {
        if ("RECRUIT".equals(unitType)) return 3;
        return 0; 
    }
    ...
    public void spawnAnimalsAroundBase(int baseX, int baseY, MapGenerator.GameMap map, GameState state) {
        ...
        for (int dx = -radius; dx <= radius; dx++) {
            for (int dy = -radius; dy <= radius; dy++) {
                ...
                if (map.terrain[nx][ny] == MapGenerator.TerrainType.MOUNTAIN) continue;
        ...
        // Spawn 4-5 Animals
        int spawnCount = MathUtils.random(4, 5);
        int spawned = 0;
    ...
    public UiInfo getObjectUi(MapGenerator.ObjectType type) {
        switch (type) {
            ...
            // --- NEW ANIMALS ---
            case HORSE: return new UiInfo("Wild Horse", horseRegion);
            case FISH: return new UiInfo("Fish School", fishRegion);
            case DEER: return new UiInfo("Forest Deer", deerRegion);
            case ZEBRA: return new UiInfo("Zebra", zebraRegion);
            // -------------------
